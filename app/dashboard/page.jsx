...
function DashboardContent() {
  ...
  const deleteReport = async (reportId, e) => {
    ...
    try {
      ...
      if (res.ok) setReports(prev => prev.filter(r => r._id !== reportId))
    } catch (err) {
    ...
    return (
      <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: 40, height: 40, border: '3px solid #0d9488', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .quick-action { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 14px 10px; background: #f8fafc; border-radius: 14px; border: 1px solid #f1f5f9; cursor: pointer; transition: all 0.2s; text-decoration: none; }
        ...
        .stats-grid { display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
        ...
        @media (max-width: 640px) {
          ...
          .quick-grid { grid-template-columns: repeat(4, 1fr) !important; gap: 8px !important; }
          .quick-action { padding: 10px 6px !important; }
          .quick-action span:last-child { font-size: 10px !important; }
          .footer-row { flex-direction: column !important; align-items: stretch !important; }
          .footer-row button, .footer-row a { width: 100% !important; }
          .reports-header { flex-wrap: wrap; gap: 10px; }
        }
      `}</style>
      ...
      <main style={{ minHeight: '100vh', background: '#f8fafc' }}>
        <div className="dash-wrap">
          ...
          <div className="fade-up" style={{ marginBottom: 28 }}>
            <p style={{ fontSize: 13, color: '#94a3b8', fontWeight: 500, marginBottom: 4 }}>{greeting} 👋</p>
            <h1 className="serif dash-title" style={{ color: '#0f172a', fontWeight: 400 }}>
              {firstName}'s Health Dashboard
            </h1>
          </div>

          {/* Top grid — score + stats */}
          <div className="top-grid fade-up" style={{ marginBottom: 16, animationDelay: '0.1s' }}>
            ...
            <div className="card" style={{ padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <svg width="140" height="140" viewBox="0 0 140 140">
                {(() => {
                  ...
                  const dash = (Math.max(reports.length ? Math.round(((reports[0]?.parameters?.filter(p => p.status?.toLowerCase() === 'normal').length || 0) / (reports[0]?.parameters?.length || 1)) * 100) : 0, 30) / 100) * circ
                  const score = reports.length ? Math.max(Math.round(((reports[0]?.parameters?.filter(p => p.status?.toLowerCase() === 'normal').length || 0) / (reports[0]?.parameters?.length || 1)) * 100), 30) : 0
              ...
              <p style={{ fontSize: 13, fontWeight: 600, color: '#0f172a', marginTop: 8, textAlign: 'center' }}>
                {(() => {
                  const score = reports.length ? Math.max(Math.round(((reports[0]?.parameters?.filter(p => p.status?.toLowerCase() === 'normal').length || 0) / (reports[0]?.parameters?.length || 1)) * 100), 30) : 0
                  return score >= 80 ? 'Excellent health' : score >= 60 ? 'Good — some attention needed' : 'Needs attention'
                })()}
              </p>
            ...
            <div className="stats-grid">
              <div className="stat-card">
                <p style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 8 }}>Total Reports</p>
                <p className="serif" style={{ fontSize: 36, color: '#0f172a', lineHeight: 1 }}>{reports.length}</p>
                <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 6 }}>
                  {reports.length === 0 ? 'Upload first report' : `Last: ${new Date(reports[0]?.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}`}
                </p>
              </div>

              <div className="stat-card" style={{ background: urgentReports > 0 ? '#fef2f2' : 'white', borderColor: urgentReports > 0 ? '#fecaca' : '#f1f5f9' }}>
                <p style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 8 }}>Urgent Flags</p>
                <p className="serif" style={{ fontSize: 36, color: urgentReports > 0 ? '#dc2626' : '#0f172a', lineHeight: 1 }}>{urgentReports}</p>
              ...
              <div className="stat-card" style={{ background: '#f0fdfa', borderColor: '#99f6e4' }}>
                <p style={{ fontSize: 11, color: '#0d9488', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 8 }}>AI Status</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <div className="dot-pulse" />
                  <p className="serif" style={{ fontSize: 20, color: '#0d9488' }}>Active</p>
                </div>
                <p style={{ fontSize: 12, color: '#0d9488', marginTop: 6 }}>Ready to analyze</p>
              </div>
          ...
          <div className="fade-up" style={{ marginBottom: 24, animationDelay: '0.2s' }}>
            <div className="quick-grid">
              {[
                ...
                { href: '/history', icon: '📈', label: 'Trends'   },
                { href: '/profile', icon: '👤', label: 'Profile'  },
                { href: '/chat',    icon: '💊', label: 'Medicine' },
              ].map((action, i) => (
          ...
          <div className="fade-up" style={{ animationDelay: '0.3s' }}>
            <div className="reports-header">
              <div>
                <h2 style={{ fontSize: 16, fontWeight: 700, color: '#0f172a' }}>Recent Reports</h2>
