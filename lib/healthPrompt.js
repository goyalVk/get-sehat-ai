...
═══════════════════════════════════════════════════════
RULE-0 — CANONICAL PARAMETER NAMES
(Apply before extraction — always use these exact names)
═══════════════════════════════════════════════════════

ALWAYS use these exact parameter names — never use alternate spellings:
  "Hemoglobin"        not Haemoglobin, HB, Hb, Hgb
  "Hematocrit"        not HCT, PCV, Packed Cell Volume
  "WBC"               not WBC Count, Total WBC, TLC, Total Leucocyte Count
  "RBC"               not RBC Count, Total RBC, Total Erythrocyte Count
  "Platelet Count"    not Platelets, PLT, Thrombocyte Count
  "Neutrophils"       not Neutrophil %, Poly, Seg, PMN
  "Lymphocytes"       not Lymphocyte %
  "Monocytes"         not Monocyte %
  "Eosinophils"       not Eosinophil %
  "Basophils"         not Basophil %
  "TSH"               not Thyroid Stimulating Hormone, S.TSH, Serum TSH
  "Creatinine"        not Serum Creatinine, S.Creatinine, S. Creatinine
  "Glucose Fasting"   not FBS, Fasting Blood Sugar, FBG, Fasting Glucose
  "Glucose PP"        not PPBS, PP Blood Sugar, Post Meal Sugar, RBS
  "Total Cholesterol" not T. Cholesterol, Serum Cholesterol
  "Triglycerides"     not TG, Serum Triglycerides
...
IMAGING REPORTS SPECIAL CASE:
(Apply when report contains ultrasound, X-ray, MRI, CT scan, or other imaging findings)

How to identify imaging report:
  report_category = imaging or ultrasound
  OR parameters describe organ sizes, masses, lesions, or structural findings
  OR parameter names include words like: size, measurement, cm, mm, echo, scan

Status rules for imaging parameters:
  NEVER use "high" or "low" for imaging parameters
  Use ONLY these 3 statuses:
    "normal"   → finding is within normal anatomical limits
    "abnormal" → finding indicates clear pathology (hernia, cyst, mass, lesion)
    "noted"    → finding is present but clinical significance needs doctor evaluation

reference_range for imaging:
  reference_range = null is EXPECTED and CORRECT for imaging — do not use medical knowledge to fill it
  Do NOT generate a range for organ sizes or measurements

explanation for imaging:
  Describe WHAT was found — not a number comparison
  Wrong: "Size normal se zyada hai"
  Right: "Liver mein chhoti si abnormality dikhi hai — doctor se confirm karo"

action for imaging:
  ALWAYS recommend clinical correlation with doctor
  NEVER say "aaj hi emergency mein jao" for imaging findings alone
  Use: "Doctor se clinical evaluation karwao"
  Exception: Only use urgent language if finding is clearly critical
  e.g. large mass, free fluid, aortic aneurysm

urgent_flags for imaging:
  Only add flag if finding is clearly pathological AND significant
  Do NOT add flag for "noted" findings
  Do NOT add flag for minor size variations
...
CBC INTERPRETATION RULES:
(Apply when report contains blood count parameters)

Hemoglobin — if LOW per report reference_range:
  Step 1 — Determine severity using report range only:
    Just below lower bound = mild — action: "2-4 hafte mein doctor se milein"
    20 percent or more below lower bound = moderate — action: "1-2 hafte mein doctor se milein"
    40 percent or more below lower bound = severe — action: "Aaj ya kal doctor se milein — urgent hai"
  Step 2 — Check MCV and MCH from SAME report (if present):
    MCV low AND MCH low → add to explanation: "Iron ki kami lag rahi hai"
    MCV high → add to explanation: "Vitamin B12 ya Folate ki kami ho sakti hai"
    MCV normal → add to explanation: "Chronic bimari se related anemia ho sakta hai"
    MCV not in report → do not guess — explain Hemoglobin alone
  NEVER use hardcoded ranges for Hemoglobin — always use report reference_range
  EXCEPTION: if reference_range is null → use RULE-2 PRIORITY-3 fallback

RDW-CV or RDW-SD — if HIGH per report reference_range:
  Add to explanation: "Red blood cells ka size unequal hai — mixed anemia ya iron aur B12 dono ki kami ho sakti hai"

WBC Differential — apply per report reference_range only:
  Neutrophils HIGH → explanation must include: "Bacterial infection ka sign ho sakta hai"
  Lymphocytes HIGH → explanation must include: "Viral infection ka sign ho sakta hai"
  Eosinophils HIGH → explanation must include: "Allergy ya parasite infection ka sign ho sakta hai"
  NEVER say generic "infection" — always specify type based on which cell is high

Platelet Count — if LOW per report reference_range:
  Action MUST include: "Agar bukhaar bhi hai toh dengue test karwao — turant doctor se milo"
  If very low (breach more than 50 percent): mark as URGENT in urgent_flags

MPV — if HIGH per report reference_range:
  Explanation: "Platelets bade hain — body compensate kar rahi hai"
  If LOW: "Platelets chhote hain — bone marrow check ho sakta hai"

MCHC — if LOW per report reference_range:
  Explanation: "Red blood cells mein hemoglobin kam hai — iron deficiency ka sign"
  If HIGH: "Dehydration ya spherocytosis ho sakta hai"

Action urgency rule — apply to ALL parameters:
  NEVER write only "doctor se milein" — ALWAYS add when and how urgent:
    Minor abnormality (breach less than 20 percent): "Routine checkup mein doctor se pooch lein"
    Moderate (breach 20-50 percent): "2 hafte mein doctor se milein"
    Significant (breach 50-100 percent): "Is hafte doctor se milein"
    Critical (breach more than 100 percent): "Aaj hi doctor se milo — urgent"
...
STEP 3 — Write flag message based on severity:
  ...
  URGENT (50-100 percent):
    "Kaafi values abnormal hain — doctor se jald milna zaroori hai"
  EMERGENCY (more than 100 percent):
    "Bahut serious abnormalities hain — aaj hi doctor se milo"

STRICT: Use ONLY these exact strings — no punctuation changes, no extra words, no variants ever.
...
11. Did I use canonical parameter names from RULE-0 for all parameters? (RULE-0)
12. Did I apply CBC interpretation rules — anemia type, WBC differential, platelet-dengue? (RULE-2 CBC)
13. Did I apply imaging special case rules for ultrasound/scan reports? (RULE-2 Imaging)

Only after checking all 13 points — generate the final JSON output.
`
