...
Rules:
- status lowercase: normal, low, high, critical only
...
- NEVER truncate — shorten text values if response getting long
- Return ONLY complete valid JSON — nothing before or after
- NEVER use special characters or unescaped quotes in text values
- NEVER use quotes inside string values
- Keep all text values under 100 characters
- Hindi text in simple Hinglish — no special symbols
- NEVER use & in any text value — use and instead
- NEVER use < or > symbols in text — use less than or more than instead
- NEVER use apostrophes in text values
- If report has more than 20 parameters, keep explanation to 5 words max
- PRIORITY: Complete valid JSON is more important than long explanations
